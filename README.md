...
PS. Этот репозиторий является только примером реализации тут нет реальной нагрузки и потокогвого стримиинга файлов для реального выполнения такой задачи хватит фоновой таски селери (тут хайп драйвен девелопмент у меня)